...
- `examples/generate_transition_refinement.rs`: builds the Rust counterpart of
  ...
  `src/migration.rs`, `src/freeze.rs`, and `src/weight.rs`, including the
  public `apply_block_transitions` and `validate_and_apply_block` operational
  APIs.
...
For block cases the summary records both the transition-only final state
(`apply_block_transitions_structural`/`apply_block_transitions`) and the
consensus-valid final state including the block-cost predicate
(`apply_valid_block_structural`/`validate_and_apply_block`).
...
cases, including final-state projection for accepted block transitions. It does
not prove SHA-256 txid collision resistance, UTXO-store backend
...
- `src/kani_proofs.rs` plus `verify_source_refinement.sh`: five PO-8 Kani
  ...
  same source-level proof module adds seventeen PO-5 bounded transition
  harnesses over `valid_tx`, `delta_tx`, `valid_block`, and block-application
