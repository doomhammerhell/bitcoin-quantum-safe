...
| Verified profile boundary + extraction/release evidence | Machine-checked Coq invariants, Coq-extracted/Rust profile-summary comparison, Rust tests, and release-binary certificate enforce that tracked post-quantum signature profiles are standards-aligned, fit the consensus witness cap, and do not become consensus-enabled without an implemented verifier/refinement boundary. This is metadata/control-plane verification, not a proof of cryptographic primitive security or compiler correctness. |
...
The executable profile bridge is now explicit. `formal/coq/extraction/PQProfileExtraction.v`
exports the profile rows and activation properties from the checked Coq model,
`formal/coq/extraction/pq_profile_refinement.ml` formats them as structured
JSON, and `examples/generate_pq_profile_refinement.rs` emits the Rust-side
counterpart from `src/pq_profile.rs`. `scripts/verification/build_extraction.sh` compares those
summaries in CI, while `scripts/verification/verify_pq_profile_refinement.sh` builds the optimized
Rust profile executable, compares its output against
`coq_pq_profile_refinement.json`, and writes
`target/pq-profile-refinement/pq_profile_refinement_certificate.json` with
source, lockfile, binary, and output hashes. This closes the operational
metadata correspondence boundary for suite activation; it still leaves FIPS
primitive security, verifier implementation correctness, and toolchain
correctness as explicit boundaries.
...
- `scripts/verification/verify_sighash_refinement.sh` builds the release Rust refinement executable,
...
- `examples/generate_transition_kernel_refinement.rs`: builds the Rust
  ...
  report witnesses.
- `scripts/verification/compare_transition_kernel_refinement.py`: compares the Coq and Rust
...
`scripts/verification/verify_txid_refinement.sh` builds the release Rust txid refinement executable,
...
`scripts/verification/verify_transition_refinement.sh` builds the release Rust transition refinement
...
`scripts/verification/verify_transition_kernel_refinement.sh` builds the optimized Rust
...
`scripts/verification/compare_transition_kernel_refinement.py`, and writes
...
`scripts/verification/verify_runtime_refinement.sh` builds the release runtime refinement executable,
...
PO-6 is now split into finite-state coverage, structural Coq theorems, and
operational refinement evidence. The TLC models still cover the configured
single-input and multi-input finite transition systems, including value
conservation and migration/freeze scenarios in the multi-input model. The new
Coq theorems close narrower but unbounded structural invariants over the
extractable association-list transition model:

- `utxo_domain`, `domain_has_no_duplicates`, and `domain_below_bool` expose the
  abstract UTXO-domain invariant checked by the witnesses.
- `utxo_total_value` exposes the aggregate economic quantity checked by the
  witnesses.
- `legacy_utxo_count`, `legacy_tx_output_count`, `frozen_utxo_count`, and
  `accepted_block_inputs_pq_or_missing` expose the structural migration/freeze
  observables checked by the witnesses.
- `delta_tx_preserves_domain_nodup` and
  `delta_tx_preserves_domain_bound` prove transaction-level preservation under
  explicit fresh-id/domain-bound preconditions.
- `apply_block_transitions_structural_preserves_domain_nodup` lifts the property
  through sequential block application.
- `apply_valid_block_structural_preserves_domain_nodup` proves that accepted
  structural block application preserves a duplicate-free final UTXO domain
  below `fresh_id + block_output_count block`.
- `apply_valid_block_structural_preserves_total_value` proves that accepted
  structural block application cannot increase total UTXO value under the same
  duplicate-free domain and fresh-id/domain-bound preconditions. This is the
  correct theorem shape for a fee-permitting rule where transaction validity
  enforces `sum(outputs) <= sum(inputs)`, not exact value equality.
- `apply_valid_block_structural_legacy_count_nonincreasing_after_announcement`
  proves that accepted structural block application cannot increase the count
  of non-PQ UTXOs after `H_a`.
- `apply_valid_block_structural_inputs_pq_after_cutover` proves that every
  input consumed by accepted structural block application after `H_c` is PQ when
  present in the evolving UTXO set.
- `apply_valid_block_structural_frozen_count_nonincreasing_after_cutover`
  proves frozen legacy UTXO count non-increase after cutover when the migration
  configuration satisfies `H_a <= H_c`.

The theorem deliberately requires the pre-state domain to be duplicate-free and
strictly below the fresh-id base. That condition isolates the txid/freshness
boundary: the Coq association-list model can prove that fresh abstract IDs do
not collide only when the caller supplies a fresh range. The proof does not
derive freshness from SHA-256, nor does it prove collision resistance of txids.

The executable bridge is intentionally per-case and semantic. `TransitionExtraction.v`
exports the domain/value/migration/freeze invariant checkers and
`transition_invariant_refinement.ml` emits structured witnesses that distinguish
accepted theorem-covered executions from rejected blocks and fresh-id boundary
cases, including pre/post total-value observations, pre/post legacy/frozen
counts, and accepted-input PQ observations. `examples/generate_transition_invariant_refinement.rs` mirrors the same
matrix through Rust's structural final-state entrypoints, and
`scripts/verification/compare_transition_invariant_refinement.py` reports nested field-level diffs
instead of relying on opaque hashes. `scripts/verification/verify_transition_invariant_refinement.sh`
builds the optimized Rust witness generator, compares it against the
Coq-extracted JSON, and emits
`target/transition-invariant-refinement/transition_invariant_refinement_certificate.json`
with toolchain, input, binary, and output hashes.

This advances PO-6 beyond finite TLC evidence for UTXO-domain uniqueness,
total-value non-increase, and structural migration/freeze monotonicity, but it
is not yet full consensus invariant preservation. Remaining open work is
cryptographic witness-validation integration, verified txid freshness, exact
monetary-supply/fee accounting if the protocol specification requires a
stronger supply theorem, and the compiler/verified-extraction boundary.
...
- `src/kani_proofs.rs` plus `scripts/verification/verify_source_refinement.sh`: five PO-8 Kani
...
- `scripts/verification/verify_compiled_refinement.sh`: builds release refinement executables,
