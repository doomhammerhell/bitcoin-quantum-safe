...
exports the domain/value invariant checkers and
...
cases, including pre/post total-value observations. `examples/generate_transition_invariant_refinement.rs` mirrors the same
...
This advances PO-6 beyond finite TLC evidence for UTXO-domain uniqueness and
total-value non-increase, but it is not yet full consensus invariant
preservation. Remaining open work is freeze/migration monotonicity as unbounded
Coq invariants, cryptographic witness-validation integration, verified txid
freshness, exact monetary-supply/fee accounting if the protocol specification
requires a stronger supply theorem, and the compiler/verified-extraction
boundary.
