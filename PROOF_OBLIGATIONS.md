...
PO-6 is now split into finite-state coverage, a structural Coq theorem, and
...
Coq theorem closes a narrower but unbounded structural invariant over the
...
exports the domain/invariant checkers and
...
cases. `examples/generate_transition_invariant_refinement.rs` mirrors the same
matrix through Rust's structural final-state entrypoints, and
`compare_transition_invariant_refinement.py` reports nested field-level diffs
instead of relying on opaque hashes. `verify_transition_invariant_refinement.sh`
...
This advances PO-6 beyond finite TLC evidence for the UTXO-domain uniqueness
invariant, but it is not yet full consensus invariant preservation. Remaining
open work is value-conservation preservation as a Coq theorem over block
application, freeze/migration monotonicity as unbounded Coq invariants,
cryptographic witness-validation integration, verified txid freshness, and the
compiler/verified-extraction boundary.
