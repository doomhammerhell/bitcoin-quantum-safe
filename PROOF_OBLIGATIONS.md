...
| Verified model + transition refinement + bounded source-level/runtime evidence | Machine-checked for the Coq UTXO transition/cost model, with Coq-extracted structural transaction/block validators and transition functions compared against the Rust implementation and its optimized release binary over deterministic edge-case matrices, Kani bounded source-level harnesses over deployed Rust `valid_tx`, `delta_tx`, and `valid_block` structural behavior, and release-binary validation of txid preimage/hash wiring plus runtime UTXO-map extensional behavior against deterministic references. This narrows implementation correspondence for UTXO transition behavior while still leaving unbounded source-level refinement, SHA-256 primitive correctness/collision resistance, runtime `HashMap` internals, cryptographic witness verification, and compiler correctness outside the artifact boundary. |
...
| PO-5 | Transition determinism | UTXO state transition must be deterministic under the same block/transaction input. | Verified model + transition refinement + bounded source-level/runtime evidence | `formal/coq/UTXOTransitions.v`, `formal/coq/extraction/TransitionExtraction.v`, `formal/coq/extraction/ExtractTransitionVectors.v`, `formal/coq/extraction/transition_refinement.ml`, `src/lib.rs`, `src/types.rs`, `src/migration.rs`, `src/freeze.rs`, `src/weight.rs`, `src/kani_proofs.rs`, `examples/generate_transition_refinement.rs`, `examples/generate_runtime_refinement.rs`, `verify_transition_refinement.sh`, `verify_runtime_refinement.sh`, `verify_source_refinement.sh` | Coq proves the structural transition and cost model. The extractable PO-5 layer exposes `delta_tx`, structural `valid_tx`, structural `valid_block`, migration/freeze checks, and cost checks. CI compares Coq-extracted summaries against Rust's `valid_tx`, `delta_tx`, `valid_block`, `check_migration_rules`, `check_no_frozen_inputs`, `cost_tx`, `weight_tx`, and `check_block_cost` over deterministic matrices covering missing input, duplicate input, value inflation, frozen legacy spends, legacy output creation after `H_a`, post-cutover legacy/taproot rejection, mixed inputs, fee-preserving multi-input transactions, sequential intra-block dependency, intra-block double spend, and block-cost boundaries. The comparison uses an explicit projection from Coq `nat` outpoint IDs to synthetic Rust `OutPoint`s; fresh IDs map to `compute_txid(tx), vout`. Kani additionally checks fifteen bounded PO-5 source-level harnesses: six `valid_tx` structural cases, five `delta_tx` removal/preservation/insertion/empty/full-spend-create cases, and four `valid_block` cases covering empty block acceptance, sequential intra-block dependency, invalid-first-transaction rejection, and intra-block double-spend rejection. Runtime `compute_txid` now hashes a domain-separated, self-delimiting `txid_preimage`, and runtime `UtxoSet` exposes deterministic canonical snapshots for extensional comparison. `verify_runtime_refinement.sh` validates the optimized runtime refinement binary against independent deterministic references for txid preimage/hash wiring and `HashMap` insert/get/remove/`delta_tx` behavior. Under `cfg(kani)`, txids use a deterministic bounded structural model and UTXO sets use a fixed-capacity finite map. This does not prove SHA-256 primitive correctness/collision resistance, runtime `HashMap` internals, PQ witness cryptographic verification, unbounded source-level refinement, or compiler/toolchain correctness. | Prove unbounded source-level Rust transition refinement, a verified SHA-256/txid implementation refinement, runtime map internals or map-refinement theorem, and compiler/toolchain correctness if full end-to-end PO-5 closure is required. |
...
that transcript. Runtime UTXO-map comparison uses `canonical_utxo_entries`, a
key-sorted snapshot that observes `HashMap` behavior extensionally rather than
...
SHA-256 invocation, checks runtime `HashMap` insert/get/remove behavior against
...
source, binary, and generated-output hashes. This narrows the txid/map runtime
boundary, but it still does not prove SHA-256 primitive correctness/collision
resistance, hash-table internals, compiler correctness, linker correctness, CPU
correctness, or OS behavior.

PO-6 remains finite-state model-checked rather than an unbounded Coq theorem.
The PO-5 transition refinement evidence and bounded source-level transition
harnesses strengthen the operational bridge to the Rust transition functions,
but they do not by themselves upgrade TLC invariant preservation to an unbounded
proof.
...
- `src/kani_proofs.rs` plus `verify_source_refinement.sh`: five PO-8 Kani
  ...
  same source-level proof module adds fifteen PO-5 bounded transition harnesses
  over `valid_tx`, `delta_tx`, and `valid_block` structural behavior.
