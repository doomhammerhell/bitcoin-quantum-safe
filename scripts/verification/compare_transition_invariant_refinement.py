#!/usr/bin/env python3
"""Compare PO-6 UTXO-domain/value/migration/freeze invariant witnesses."""
...
DEFAULT_COQ = "formal/coq/extraction/coq_transition_invariant_refinement.json"
DEFAULT_RUST = "rust_transition_invariant_refinement.json"
MAX_DIFFS = 200
...
def diff_values(path: str, coq: Any, rust: Any, diffs: list[str]) -> None:
    ...
    if isinstance(coq, dict):
        for key in sorted(set(coq) | set(rust)):
            if len(diffs) >= MAX_DIFFS:
                return
...
def build_case_map(label: str, cases: Any) -> tuple[dict[str, dict[str, Any]], list[str]]:
    if not isinstance(cases, list):
        return {}, [f"{label}: expected list, found {type(cases).__name__}"]
...
def compare_case_set(coq_cases: Any, rust_cases: Any) -> list[str]:
    diffs: list[str] = []
    coq_by_key, coq_shape_diffs = build_case_map("cases.Coq", coq_cases)
    rust_by_key, rust_shape_diffs = build_case_map("cases.Rust", rust_cases)
    ...
    for key in sorted(set(coq_by_key) | set(rust_by_key)):
        if key not in coq_by_key:
            diffs.append(f"cases.{key}: missing in Coq")
            continue
        if key not in rust_by_key:
            diffs.append(f"cases.{key}: missing in Rust")
            continue
        diff_values(f"cases.{key}", coq_by_key[key], rust_by_key[key], diffs)
        if len(diffs) >= MAX_DIFFS:
            break
...
def compare_witnesses(coq: dict[str, Any], rust: dict[str, Any]) -> list[str]:
    ...
    diffs.extend(compare_case_set(coq.get("cases"), rust.get("cases")))
...
def main() -> int:
    ...
    if diffs:
        print("=== TRANSITION INVARIANT REFINEMENT SEMANTIC DIFF ===")
    ...
    print("=== SUCCESS ===")
    print("PO-6 UTXO-domain/value/migration/freeze invariant witnesses match.")
