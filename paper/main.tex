...
PO-5 & Transition determinism & \textbf{Verified model + txid/transition/source/runtime evidence} & Coq + extracted txid/transition refinement + Kani guards + runtime txid/store validation \\
...
``Verified model + txid/transition/source/runtime evidence'' = machine-checked proof for the Coq UTXO transition model and txid preimage transcript, plus Coq-extracted vs Rust txid and structural transaction/block transition/final-state refinement, release-binary validation over deterministic structural edge-case matrices, nineteen Kani bounded source-level PO-5 harnesses over deployed \texttt{valid\_tx\_structural}, \texttt{delta\_tx}, \texttt{valid\_block\_structural}, and structural block-application behavior, and release-binary validation of txid preimage/hash wiring plus runtime UTXO-store extensional behavior against independent deterministic references. SHA-256 primitive correctness/collision resistance, store backend internals, cryptographic witness verification, compiler correctness, and the remaining verified-extraction path remain outside this artifact boundary.
