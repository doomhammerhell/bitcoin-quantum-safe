...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{PO-6 (Structural UTXO-domain preservation):} valid structural block application preserves a duplicate-free UTXO domain below the next fresh-id bound, assuming the pre-state domain is duplicate-free and below the current fresh-id base.
  Machine-checked as \texttt{apply\_valid\_block\_structural\_preserves\_domain\_nodup}, with transaction-level preservation discharged by \texttt{delta\_tx\_preserves\_domain\_nodup} and \texttt{delta\_tx\_preserves\_domain\_bound}.
...
PO-6 & Invariant preservation & \textbf{Model-checked + structural Coq invariant evidence} & TLA+/TLC + Coq domain theorem + extracted/Rust invariant witnesses \\
...
``Model-checked + structural Coq invariant evidence'' = TLC finite-state invariant checking plus a Coq theorem that accepted structural block application preserves duplicate-free abstract UTXO domains below the next fresh-id bound under an explicit pre-state freshness/domain-bound precondition; Coq-extracted invariant witnesses are compared against Rust structural final-state observations and optimized release binaries, including non-applicability cases for fresh-id/collision boundaries.
