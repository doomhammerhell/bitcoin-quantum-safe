...
TLC checks four invariants ($\textsf{NoDoubleSpend}$, $\textsf{ValueBound}$, $\textsf{StateConsistency}$, $\textsf{MigrationMonotonicity}$) plus the temporal property $\textsf{PQFracNonDecreasing}$.
...
\paragraph{Module 3: varint axiom consistency (\texttt{VarintConcrete.v}).}
This module provides a concrete varint implementation (single-byte encoding) and proves that all 6 varint axioms from \texttt{SpendPredPQ.v} are satisfied, establishing that the axiom system is consistent (not vacuously true).
The proofs are packaged as a \texttt{VarintAxioms} record, and a concrete witness round-trip is proved (\texttt{parse\_serialize\_concrete\_round\_trip}).
This addresses PO-8 at the axiom level: the abstract interface has at least one correct implementation.
...
PO-8 & Implementation correspondence & \textbf{Partial} & Coq (\texttt{VarintConcrete.v}) + Rust PBT \\
\bottomrule
\end{tabular}
...
The current evidence for PO-8 consists of (i) axiom consistency proved in Coq, (ii) 16 Rust property-based tests that verify each Coq theorem holds in the implementation, and (iii) 3 adversarial boundary tests that exercise the security properties from Theorems~\ref{thm:reduction} and~\ref{thm:complete-auth} (commitment second-preimage resistance, cross-input replay resistance, cross-transaction replay resistance).
Full PO-8 closure would require Coq extraction to Rust or a verified translation --- a substantial but well-scoped effort that builds on the foundation established here.
...
The formal artifacts --- TLC model checking of UTXO transitions (two models: single-input and multi-input with value conservation; zero structural invariant violations; concrete migration dilemma counterexample), Coq-mechanized proofs of spend predicate totality, determinism, and parse canonicality (strengthened to full witness identity via varint-based encoding), Coq-mechanized proofs of transition determinism (PO-5) and cost boundedness with exact weight equality (PO-7), a concrete varint implementation proving axiom consistency (PO-8 foundation), and a reference implementation in Rust with FIPS\,204 ML-DSA-44 signatures, 25 property-based tests covering all 16 correctness properties plus 9 sighash commitment tests, and 3 adversarial boundary tests --- provide machine-checked and empirically validated evidence that the framework is not merely described but verified.
