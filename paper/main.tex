...
(i) consensus invariants (no double spend, state consistency, determinism) are preserved across all valid transitions (Theorems~\ref{thm:inv-preservation},~\ref{thm:inv-blk});
...
(v) no cryptographic-only protocol transition can simultaneously preserve safety and liveness for all outputs when lost keys exist (Theorems~\ref{thm:no-free-migration},~\ref{thm:dilemma}).
...
(d) witness parsing is canonical, total, and deterministic (PO-1 through PO-3);
(e) the mechanized spend predicate corresponds to the implementation used by consensus nodes (PO-8).
...
A credible plan couples an executable model (for counterexample finding) with mechanized semantics (for proof of critical components).

\subsection{TLA+ model of UTXO transitions}
The minimal TLA+ specification includes explicit $utxo$ state, deterministic transitions, and invariants.

\begin{lstlisting}[caption={TLA+ skeleton: UTXO, validation, and PQ authorization},label={lst:tla}]
---- MODULE BitcoinPQ ----
VARIABLES utxo, chain
...
ValidTx(tx, utxo) ==
  /\ NoDupInputs(tx)
  /\ InputsExist(tx, utxo)
  /\ ValueConservation(tx, utxo)
  /\ \A i \in Inputs(tx):
       SpendPredPQ(Lock(utxo[PrevOut(tx,i)]),
                   Sighash(tx,i),
                   Wit(tx,i))
...
Invariant ==
  /\ NoDoubleSpend(chain)
  /\ AuthIntegrityPQ(chain)
  /\ Determinism(utxo)
==========================
\end{lstlisting}

\subsection{Explicit proof obligations}\label{sec:proof-obligations}

We enumerate the concrete proof obligations that a formal verification effort must discharge.
These serve both as a specification for mechanization and as a checklist for hostile review.

\begin{enumerate}[label=\textbf{PO-\arabic*},leftmargin=3em]
  \item \textbf{Spend predicate totality:}
  $\forall P,m,w:\; \SpendPredPQ(P,m,w)\in\{0,1\}$.
  The predicate never diverges, raises exceptions, or returns $\bot$.

  \item \textbf{Spend predicate determinism:}
  $\forall P,m,w:\; \SpendPredPQ(P,m,w)$ yields the same result on every evaluation.
  No dependence on mutable state, randomness, or evaluation order.

  \item \textbf{Witness parsing canonicality:}
  $\textsf{Parse}:\Witness\to(\mathsf{pk},\sigma)\cup\{\bot\}$ is a total, deterministic, injective function on its accepting domain.
  No two distinct byte sequences parse to the same $(\mathsf{pk},\sigma)$ pair.

  \item \textbf{Sighash commitment:}
  The concrete sighash implementation satisfies Definition~\ref{def:sighash-commit} (injectivity, cross-input separation, field coverage).

  \item \textbf{Transition determinism:}
  $\forall U,tx:\; \ValidTx(U,tx)=1 \Rightarrow \exists!\, U':\; U'=\DeltaTx(U,tx)$.

  \item \textbf{Invariant preservation:}
  Theorems~\ref{thm:inv-preservation} and~\ref{thm:inv-blk} hold for the concrete instantiation.

  \item \textbf{Cost boundedness:}
  $\forall tx:\; \ValidTx(U,tx)=1 \Rightarrow \Cost(tx)\le\alpha\cdot\weight(tx)$ for the chosen PQ scheme parameters and the engineering constant $\alpha$.

  \item \textbf{Implementation correspondence:}
  The mechanized spend predicate is semantically equivalent to the C++/Rust implementation used by consensus nodes.
\end{enumerate}

\begin{remark}
PO-1 through PO-5 are amenable to mechanization in Coq or Isabelle~\cite{bertot2004coq,nipkow2002isabelle}.
PO-6 follows from PO-1 through PO-5 by the proofs in Section~\ref{sec:invariant-preservation}.
PO-7 requires concrete parameterization (scheme choice, encoding format).
PO-8 requires extraction or bisimulation against production code and is the most labor-intensive obligation.
\end{remark}

\subsection{Mechanizing Script/witness semantics}
Network-level invariants are best explored via model checking, but Script semantics and witness parsing demand mechanization to avoid consensus bugs.
A pragmatic route is to mechanize the spend predicate and parsing rules in Coq/Isabelle~\cite{bertot2004coq,nipkow2002isabelle}, and prove correspondence against a reference implementation.
The deliverable is narrow: ``the PQ spend predicate is deterministic, total, and equivalent to the implementation used by nodes''.

\subsection{Model checking scope}
The TLA+ model (Listing~\ref{lst:tla}) is designed for finite-state exploration with TLC.
The model should be instantiated with:
\begin{itemize}[leftmargin=2em]
  \item a small UTXO set ($|\dom(U_0)|\le 10$);
  \item bounded transaction and block counts;
  \item both PQ-locked and legacy outputs (to verify that the migration cutover correctly rejects legacy spends after height $H_c$).
\end{itemize}
The invariants to check are $\textsf{NoDoubleSpend}$, $\textsf{StateConsistency}$, $\textsf{Determinism}$, and the migration enforcement predicate.
Any counterexample found by TLC constitutes a concrete consensus bug.
