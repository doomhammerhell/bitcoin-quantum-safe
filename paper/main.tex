...
(vii) the UTXO transition function is deterministic and preserves the no-double-spend invariant --- machine-checked in Coq and connected to the Rust transition implementation by Coq-extracted structural transition refinement matrices (PO-5);
...
(e) the structural UTXO transition correspondence between Coq and Rust is supported by Coq-extracted transaction/block transition summaries and release-binary validation over deterministic edge-case matrices, while txid collision resistance, Rust map internals, cryptographic witness verification, and compiler correctness remain explicit boundaries (PO-5).
...
PO-5 & Transition determinism & \textbf{Verified model + transition evidence} & Coq + extracted transition refinement \\
...
``Verified model + transition evidence'' = machine-checked proof for the Coq UTXO transition model, plus Coq-extracted vs Rust transaction/block transition refinement and release-binary validation over deterministic structural edge-case matrices.
...
The formal artifacts --- TLC model checking of UTXO transitions (two models: single-input with 492 states and multi-input with 58{,}237 states and value conservation; zero structural invariant violations; concrete migration dilemma counterexample in both models), Coq-mechanized proofs of spend predicate totality, determinism, and parse canonicality (strengthened to full witness identity via axiomatized varint model with concrete bounded compact-size discharge, consensus-size guard, golden test vectors, exhaustive varint refinement, and witness-level consensus-domain operational-trace refinement matrices), Kani source-level bounded verification of the deployed Rust witness parser layout/canonicality relation, release-binary translation validation against the Coq-extracted PO-8 summaries, Coq-mechanized proofs of the Sighash v2 commitment model under a SHA-256 collision-resistance axiom plus Coq-extracted vs Rust transcript/preimage refinement for PO-4, transition determinism with Coq-extracted vs Rust structural transition refinement for PO-5, and cost boundedness with exact weight equality (PO-7), and a reference implementation in Rust with FIPS\,204 ML-DSA-44 signatures, 34 property-based tests including 9 sighash commitment properties, 28 PO-8 correspondence tests with bounded golden vectors, a Rust consensus-size guard, and 3 adversarial boundary tests --- provide machine-checked and empirically validated evidence while leaving cryptographic primitive implementation correctness, txid collision-resistance implementation correctness, Rust map internals, and compiler correctness as explicit implementation-correspondence boundaries.
