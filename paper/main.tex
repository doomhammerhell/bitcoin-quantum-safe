...
(vii) the UTXO transition function is deterministic and preserves the no-double-spend invariant --- machine-checked in Coq and connected to the Rust transition implementation by Coq-extracted txid/structural transition/final-state refinement matrices plus bounded source-level \texttt{valid\_tx}, \texttt{delta\_tx}, \texttt{valid\_block}, and block-application harnesses (PO-5);
...
The extraction layer \texttt{TxidExtraction.v}/\texttt{txid\_refinement.ml} summarizes the txid transcript over deterministic count-delimited transaction matrices. \texttt{TransitionExtraction.v} and \texttt{transition\_refinement.ml} summarize transition functions and executable block-application final states over deterministic transaction, block, and cost matrices. The Coq development proves that \texttt{apply\_block\_transitions\_structural} is equivalent to the transition-validity boolean and that \texttt{apply\_valid\_block\_structural} returns \texttt{Some} final state exactly when \texttt{valid\_block\_structural} accepts. The matching Rust executables call deployed \texttt{txid\_preimage}, \texttt{valid\_tx}, \texttt{delta\_tx}, \texttt{apply\_block\_transitions}, \texttt{validate\_and\_apply\_block}, \texttt{valid\_block}, migration, freeze, and weight functions. The matrix covers count-delimited txid transcripts, missing inputs, duplicate inputs, value inflation, legacy output creation after $H_a$, frozen legacy/taproot spends at $H_c$, mixed PQ/legacy inputs, fee-preserving multi-input transactions, sequential intra-block dependencies, intra-block double spends, projected final UTXO states, and exact/over-limit block cost. Kani additionally checks seventeen bounded source-level PO-5 harnesses over deployed Rust control flow: six \texttt{valid\_tx} structural cases, five \texttt{delta\_tx} removal/preservation/insertion/empty/full-spend-create cases, four \texttt{valid\_block} empty/sequential/rejection cases, and two block-application final-state/projection cases. Under \texttt{cfg(kani)}, txids use a deterministic bounded structural model and the UTXO set uses a fixed-capacity finite map. The runtime path exposes a domain-separated, self-delimiting \texttt{txid\_preimage}, computes \texttt{compute\_txid} as SHA-256 of that transcript, exposes \texttt{apply\_block\_transitions}/\texttt{validate\_and\_apply\_block} as final-state APIs, and exposes key-sorted UTXO snapshots through an explicit \texttt{UtxoStore} extensional contract. Release-binary validators compare txid preimage, transition/final-state, and runtime store operations against Coq-extracted or independent deterministic references. The comparison is over an explicit projection from Coq natural-number outpoint identifiers to Rust \texttt{OutPoint}s; it is not a proof of SHA-256 primitive correctness or collision resistance, store backend internals, cryptographic witness verification, or compiler correctness. The selected unbounded-refinement path is Coq-first verified/extracted transition core plus thin Rust adapters; Verus, Creusot, or Prusti are reserved for adapter/store obligations rather than as the primary proof path for consensus transition semantics.
