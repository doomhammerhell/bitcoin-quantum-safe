...
(a) the sighash function satisfies the commitment property (Definition~\ref{def:sighash-commit}), including consensus-semantic injectivity --- supported by executable evidence (property-based tests with tagged hashes) but not mechanized in Coq (PO-4);
...
(d) the mechanized spend predicate corresponds to the implementation used by consensus nodes --- supported by axiom consistency proofs and property-based correspondence tests but not by extraction or bisimulation (PO-8).
...
The witness parse function uses an axiomatized varint encoder/decoder (6 axioms capturing round-trip, positive length, trailing-data tolerance, prefix determinism, canonical uniqueness, and consumed-length equality) that is faithful to Bitcoin's compact-size encoding and to the Rust implementation in \texttt{src/encoding.rs}.
Properties proved:
...
Additionally, the module includes \textbf{golden test vectors} generated via \texttt{Compute} that are verified byte-for-byte against the Rust implementation (including the ML-DSA-44 public key length 1{,}312 and signature length 2{,}420).
This addresses PO-8 at the encoding level: the Coq model uses the same multi-byte varint format as the Rust code, and the golden vectors provide concrete correspondence evidence.
...
PO-4 & Sighash commitment & \textbf{Executable evidence} & Rust PBT (9 property tests) \\
...
PO-8 & Implementation correspondence & \textbf{Strong evidence} & Coq (\texttt{VarintConcrete.v}) + Rust PBT \\
\bottomrule
\end{tabular}
...
``Partial'' = axiom consistency proved in Coq; implementation correspondence verified by property-based tests but not by extraction or bisimulation.
``Strong evidence'' = concrete multi-byte varint model in Coq matches the Rust implementation byte-for-byte (golden test vectors); all axioms proved for the concrete model; 19 Rust property-based tests verify each Coq theorem holds in the implementation; full extraction/bisimulation remains as future work.}
\label{tab:po-status}
\end{table}
...
PO-4 (sighash commitment) is supported by executable evidence: a \texttt{verify\_sighash\_commitment\_property} function that programmatically checks injectivity, cross-input separation, and field coverage for any transaction, plus 9 property-based tests exercising these properties across hundreds of randomly generated transactions with BIP\,340-style tagged hashes for domain separation.
The remaining gap is PO-8 (full implementation correspondence): establishing a formal bisimulation between the Coq model and the Rust implementation.
The current evidence for PO-8 consists of (i) all 6 varint axioms proved for a concrete multi-byte encoding in Coq that uses the same Bitcoin compact-size format as the Rust code, (ii) golden test vectors generated by Coq \texttt{Compute} and verified byte-for-byte against the Rust implementation (including ML-DSA-44 key/signature lengths), (iii) 19 Rust property-based tests that verify each Coq theorem holds in the implementation, and (iv) 3 adversarial boundary tests that exercise the security properties from Theorems~\ref{thm:reduction} and~\ref{thm:complete-auth} (commitment second-preimage resistance, cross-input replay resistance, cross-transaction replay resistance).
Full PO-8 closure would require Coq extraction to OCaml/Rust or a verified translation --- a substantial but well-scoped effort that builds on the foundation established here.
...
The formal artifacts --- TLC model checking of UTXO transitions (two models: single-input with 492 states and multi-input with 58{,}237 states and value conservation; zero structural invariant violations; concrete migration dilemma counterexample in both models), Coq-mechanized proofs of spend predicate totality, determinism, and parse canonicality (strengthened to full witness identity via axiomatized varint model with concrete Bitcoin compact-size discharge and golden test vectors), Coq-mechanized proofs of transition determinism (PO-5) and cost boundedness with exact weight equality (PO-7), and a reference implementation in Rust with FIPS\,204 ML-DSA-44 signatures, 25 property-based tests covering all 16 correctness properties plus 9 sighash commitment tests, 19 PO-8 correspondence tests with golden vectors, and 3 adversarial boundary tests --- provide machine-checked and empirically validated evidence that the framework is not merely described but verified.
