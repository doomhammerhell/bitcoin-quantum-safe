...
(vii) the UTXO transition function is deterministic and preserves the no-double-spend invariant --- machine-checked in Coq (PO-5);
...
(a) the deployed sighash implementation satisfies the commitment property (Definition~\ref{def:sighash-commit}), including consensus-semantic injectivity --- the Coq model of Sighash v2 discharges PO-4 under a SHA-256 collision-resistance axiom, while full Rust-to-Coq refinement remains an implementation-correspondence obligation;
...
The Coq development now proves the modeled Sighash v2 commitment property under a SHA-256 collision-resistance axiom (PO-4 in Section~\ref{sec:proof-obligations}).
What remains outside that proof is an implementation-refinement theorem showing that the deployed Rust byte construction is extensionally equal to the Coq model.
Our results should be read as: ``\emph{if} the deployed sighash corresponds to the verified model and SHA-256 satisfies the stated collision-resistance axiom, \emph{then} the stated security guarantees hold.''
...
PO-4 & Sighash commitment & \textbf{Verified model + exec. evidence} & Coq model + Rust PBT \\
PO-5 & Transition determinism & \textbf{Verified} & Coq (\texttt{UTXOTransitions.v}) \\
...
PO-4 (sighash commitment) is proved for the Coq Sighash v2 model under a SHA-256 collision-resistance axiom and supported at the implementation layer by executable evidence: a \texttt{verify\_sighash\_commitment\_property} function that programmatically checks injectivity, cross-input separation, and field coverage for consensus-encodable transactions, plus 9 property-based tests exercising these properties across hundreds of randomly generated transactions with BIP\,340-style tagged hashes for domain separation.
...
The formal artifacts --- TLC model checking of UTXO transitions (two models: single-input with 492 states and multi-input with 58{,}237 states and value conservation; zero structural invariant violations; concrete migration dilemma counterexample in both models), Coq-mechanized proofs of spend predicate totality, determinism, and parse canonicality (strengthened to full witness identity via axiomatized varint model with concrete bounded compact-size discharge, consensus-size guard, golden test vectors, exhaustive varint refinement, and witness-level consensus-domain operational-trace refinement matrices), Kani source-level bounded verification of the deployed Rust witness parser layout/canonicality relation, release-binary translation validation against the Coq-extracted PO-8 summaries, Coq-mechanized proofs of the Sighash v2 commitment model under a SHA-256 collision-resistance axiom (PO-4), transition determinism (PO-5), and cost boundedness with exact weight equality (PO-7), and a reference implementation in Rust with FIPS\,204 ML-DSA-44 signatures, 25 property-based tests covering all 16 correctness properties plus 9 sighash commitment tests, 20 PO-8 correspondence tests with bounded golden vectors, a Rust consensus-size guard, and 3 adversarial boundary tests --- provide machine-checked and empirically validated evidence while leaving compiler correctness as the primary explicit implementation-correspondence boundary.
