...
PO-4 & Sighash commitment & \textbf{Verified model + transcript evidence} & Coq model + extracted transcript + Rust PBT \\
PO-5 & Transition determinism & \textbf{Verified model + txid/transition/source/runtime evidence} & Coq + extracted txid/transition/kernel-report refinement + Kani guards + runtime txid/store validation \\
PO-6 & Invariant preservation & \textbf{Model-checked + structural Coq invariant evidence} & TLA+/TLC + Coq domain/value theorems + extracted/Rust invariant witnesses \\
PO-7 & Cost boundedness & \textbf{Verified} & Coq (\texttt{UTXOTransitions.v}) \\
PO-8 & Implementation correspondence & \textbf{Bounded + source + binary evidence} & Coq extraction + Kani + binary validation + Rust PBT \\
\bottomrule
\end{tabular}
\caption{Status of proof obligations.
``Verified'' = machine-checked proof in Coq.
``Verified (strengthened)'' = machine-checked proof that is strictly stronger than the original obligation (full witness identity rather than region matching).
``Model-checked'' = exhaustively verified by TLC over finite state space.
``Executable evidence'' = property-based tests verifying the property across hundreds of randomly generated inputs.
``Verified model + transcript evidence'' = machine-checked proof for the Coq model under explicit cryptographic axioms, plus Coq-extracted vs Rust transcript/preimage refinement and executable tests of the concrete implementation.
``Verified model + txid/transition/source/runtime evidence'' = machine-checked proof for the Coq UTXO transition model and txid preimage transcript, plus Coq-extracted vs Rust txid, structural transaction/block transition/final-state, and \texttt{CoqExtractedTransitionKernel}/\texttt{DeployedTransitionKernel} per-case report refinement, release-binary validation over deterministic structural edge-case matrices, twenty-one Kani bounded source-level PO-5 harnesses over deployed \texttt{valid\_tx\_structural}, \texttt{delta\_tx}, \texttt{valid\_block\_structural}, structural block-application behavior, and the \texttt{TransitionKernel} adapter projection, and release-binary validation of txid preimage/SHA-256 wiring plus runtime UTXO-store extensional behavior against independent deterministic references. SHA-256 primitive correctness/collision resistance, store backend internals, cryptographic witness verification, compiler correctness, and the remaining verified-extraction path remain outside this artifact boundary.
``Model-checked + structural Coq invariant evidence'' = TLC finite-state invariant checking plus Coq theorems that accepted structural block application preserves duplicate-free abstract UTXO domains below the next fresh-id bound and cannot increase total UTXO value under an explicit pre-state freshness/domain-bound precondition; Coq-extracted invariant witnesses are compared against Rust structural final-state observations and optimized release binaries, including non-applicability cases for fresh-id/collision boundaries.
``Bounded + source + binary evidence'' = the concrete Coq varint model covers the single-byte and \texttt{0xFD}/u16 compact-size ranges; concrete parser/serializer canonicality and parse injectivity are machine-checked for that bounded model; the protocol witness-size cap is proved/checked to remain within that domain; Coq-extracted varint encode/decode behavior is exhaustively compared against Rust over all modeled values; Coq-extracted witness serialize/parse/consensus-domain parse/canonicality behavior and parser decision traces are compared against Rust over a deterministic boundary/rejection matrix and a symbolic bounded state-space; witness bytes generated by the Coq-extracted serializer match the Rust implementation byte-for-byte on seven golden vectors; all six varint axioms are proved for that bounded model; 28 Rust tests exercise the corresponding implementation properties; five Kani harnesses verify source-level bounded parser/layout/canonicality alignment over symbolic Rust inputs; and optimized release refinement binaries are executed and required to reproduce the Coq-extracted summaries with source/binary hashes recorded. Compiler correctness remains outside the artifact boundary.}
\label{tab:po-status}
\end{table}
