...
(vii) the UTXO transition function is deterministic and preserves the no-double-spend invariant --- machine-checked in Coq and connected to the Rust transition implementation by Coq-extracted structural transition refinement matrices plus bounded source-level \texttt{valid\_tx} guard harnesses (PO-5);
...
(e) the structural UTXO transition correspondence between Coq and Rust is supported by Coq-extracted transaction/block transition summaries, release-binary validation over deterministic edge-case matrices, and bounded source-level \texttt{valid\_tx} guard verification, while full \texttt{delta\_tx}/\texttt{valid\_block} source refinement, txid collision resistance, Rust map internals, cryptographic witness verification, and compiler correctness remain explicit boundaries (PO-5).
...
\paragraph{Module 2: UTXO transitions and cost model (\texttt{UTXOTransitions.v}).}
...
The extraction layer \texttt{TransitionExtraction.v} and \texttt{transition\_refinement.ml} summarize these functions over deterministic transaction, block, and cost matrices. The matching Rust executable calls the deployed \texttt{valid\_tx}, \texttt{delta\_tx}, \texttt{valid\_block}, migration, freeze, and weight functions. The matrix covers missing inputs, duplicate inputs, value inflation, legacy output creation after $H_a$, frozen legacy/taproot spends at $H_c$, mixed PQ/legacy inputs, fee-preserving multi-input transactions, sequential intra-block dependencies, intra-block double spends, and exact/over-limit block cost. Kani additionally checks fifteen bounded source-level PO-5 harnesses over deployed Rust control flow: six \texttt{valid\_tx} structural cases, five \texttt{delta\_tx} removal/preservation/insertion/empty/full-spend-create cases, and four \texttt{valid\_block} empty/sequential/rejection cases. Under \texttt{cfg(kani)}, txids use a deterministic bounded structural model and the UTXO set uses a fixed-capacity finite map. The runtime path separately exposes a domain-separated, self-delimiting \texttt{txid\_preimage}, computes \texttt{compute\_txid} as SHA-256 of that transcript, and exposes key-sorted UTXO snapshots for extensional \texttt{HashMap} comparison. A release-binary runtime validator compares these txid and map operations against independent deterministic references. The comparison is over an explicit projection from Coq natural-number outpoint identifiers to Rust \texttt{OutPoint}s; it is not a proof of unbounded source-level transition refinement, SHA-256 primitive correctness or collision resistance, \texttt{HashMap} internals, cryptographic witness verification, or compiler correctness.
...
PO-5 & Transition determinism & \textbf{Verified model + transition/source/runtime evidence} & Coq + extracted transition refinement + Kani guards + runtime txid/map validation \\
...
``Verified model + transition/source/runtime evidence'' = machine-checked proof for the Coq UTXO transition model, plus Coq-extracted vs Rust transaction/block transition refinement, release-binary validation over deterministic structural edge-case matrices, fifteen Kani bounded source-level PO-5 harnesses over deployed \texttt{valid\_tx}, \texttt{delta\_tx}, and \texttt{valid\_block} behavior, and release-binary validation of txid preimage/hash wiring plus runtime UTXO-map extensional behavior against independent deterministic references. Unbounded source-level transition refinement, SHA-256 primitive correctness/collision resistance, Rust map internals, cryptographic witness verification, and compiler correctness remain outside this artifact boundary.
...
PO-5 is supported beyond the pure Coq transition theorem by an extracted transition-refinement matrix: Coq structural \texttt{valid\_tx}, \texttt{delta\_tx}, structural \texttt{valid\_block}, migration/freeze checks, and cost checks are summarized and compared against Rust's deployed transition functions over missing-input, duplicate-input, value-inflation, frozen-spend, post-activation/post-cutover, sequential intra-block, double-spend, and block-cost boundary cases. The release-binary validator emits a certificate for this comparison. Fifteen Kani bounded source-level harnesses further check deployed transition behavior: six \texttt{valid\_tx} structural guard cases, five \texttt{delta\_tx} removal/preservation/insertion/empty/full-spend-create cases, and four \texttt{valid\_block} empty/sequential/rejection cases. A separate runtime validator emits a certificate for \texttt{txid\_preimage}, \texttt{compute\_txid}, canonical UTXO snapshots, and runtime \texttt{HashMap} insert/get/remove/\texttt{delta\_tx} behavior against independent deterministic references. The remaining PO-5 implementation boundary is unbounded source-level transition refinement, SHA-256 primitive correctness/collision resistance, \texttt{HashMap} internals, cryptographic witness verification, and compiler/toolchain correctness.
...
Full compiler-correctness closure remains a separate artifact class. For PO-4, the transcript/preimage serialization layer is extracted and compared against Rust, while SHA-256 itself remains an explicit cryptographic primitive boundary. For PO-5, structural transition behavior is extracted and compared against Rust over deterministic matrices, bounded source-level transition behavior for deployed \texttt{valid\_tx}, \texttt{delta\_tx}, and \texttt{valid\_block} is checked by Kani, and txid/map runtime behavior is release-validated against deterministic references, while unbounded source-level transition refinement, SHA-256/hash-map/compiler boundaries remain explicit. For PO-8, the consensus-valid witness subset is bounded by u16, varint-level refinement is exhaustive, witness-level extracted-function plus consensus-domain operational-trace refinement is checked by CI, the Rust source parser is bounded-verified by Kani, and the release binaries are translation-validated over the modeled PO-8 domains. Extending the Coq model to \texttt{0xFE}/\texttt{0xFF} remains necessary only for a general-purpose CompactSize verification claim or if the witness cap is raised above 65{,}535 bytes.
...
The formal artifacts --- TLC model checking of UTXO transitions (two models: single-input with 492 states and multi-input with 58{,}237 states and value conservation; zero structural invariant violations; concrete migration dilemma counterexample in both models), Coq-mechanized proofs of spend predicate totality, determinism, and parse canonicality (strengthened to full witness identity via axiomatized varint model with concrete bounded compact-size discharge, consensus-size guard, golden test vectors, exhaustive varint refinement, and witness-level consensus-domain operational-trace refinement matrices), Kani source-level bounded verification of the deployed Rust witness parser layout/canonicality relation and PO-5 transition behavior over \texttt{valid\_tx}, \texttt{delta\_tx}, and \texttt{valid\_block}, release-binary translation validation against the Coq-extracted PO-8 summaries, Coq-mechanized proofs of the Sighash v2 commitment model under a SHA-256 collision-resistance axiom plus Coq-extracted vs Rust transcript/preimage refinement for PO-4, transition determinism with Coq-extracted vs Rust structural transition refinement for PO-5, runtime txid/map refinement validation for the deployed transition path, and cost boundedness with exact weight equality (PO-7), and a reference implementation in Rust with FIPS\,204 ML-DSA-44 signatures, 34 property-based tests including 9 sighash commitment properties, 28 PO-8 correspondence tests with bounded golden vectors, a Rust consensus-size guard, and 3 adversarial boundary tests --- provide machine-checked and empirically validated evidence while leaving cryptographic primitive implementation correctness, unbounded source-level transition refinement, SHA-256 collision-resistance implementation correctness, Rust map internals, and compiler correctness as explicit implementation-correspondence boundaries.
