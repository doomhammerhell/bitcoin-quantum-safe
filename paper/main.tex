...
(i) consensus invariants (no double spend, state consistency, determinism) are preserved across all valid transitions (Theorems~\ref{thm:inv-preservation},~\ref{thm:inv-blk}), confirmed by exhaustive TLC model checking (Section~\ref{sec:tlc-results}) and strengthened for structural UTXO-domain uniqueness and total-value non-increase by Coq theorems under an explicit fresh-id/domain-bound precondition;
...
(vi) the PQ spend predicate is total, deterministic, and its witness parsing is canonical --- machine-checked in Coq (Section~\ref{sec:coq-results}, proof obligations PO-1, PO-2, PO-3, with PO-3 strengthened to full witness identity via varint-based encoding);
(vii) the UTXO transition function is deterministic and preserves the no-double-spend invariant --- machine-checked in Coq and connected to explicit Rust structural transition entrypoints by Coq-extracted txid/structural transition/final-state refinement matrices plus per-case TransitionKernel report witnesses and bounded source-level \texttt{valid\_tx\_structural}, \texttt{delta\_tx}, \texttt{valid\_block\_structural}, structural block-application, and \texttt{TransitionKernel} adapter harnesses (PO-5);
(viii) the cost function satisfies $\Cost(tx) = \weight(tx)$ with exact equality --- machine-checked in Coq (PO-7).

\paragraph{What is assumed.}
The following are explicit axioms or implementation obligations, \emph{not} proven in this paper:
(a) the deployed sighash implementation satisfies the commitment property (Definition~\ref{def:sighash-commit}), including consensus-semantic injectivity --- the Coq model of Sighash v2 discharges PO-4 under a SHA-256 collision-resistance axiom, and the deterministic Rust preimage transcript construction is checked against a Coq-extracted transcript function, while SHA-256 implementation correctness and compiler correctness remain outside the artifact boundary;
...
(d) the mechanized spend predicate corresponds to the implementation used by consensus nodes --- supported by bounded Coq-to-OCaml serializer extraction, golden-vector comparison, property-based correspondence tests, Kani source-level bounded parser-refinement harnesses, and release-binary translation validation against the Coq-extracted summaries, but not by a proof of compiler correctness (PO-8);
(e) the structural UTXO transition correspondence between Coq and Rust is supported by Coq-extracted txid and transaction/block transition summaries, release-binary validation over deterministic edge-case matrices, bounded source-level \texttt{valid\_tx}/\texttt{delta\_tx}/\texttt{valid\_block} verification, and runtime \texttt{UtxoStore} extensional validation, while txid collision resistance, SHA-256 primitive correctness, store backend internals, cryptographic witness verification, and compiler correctness remain explicit boundaries (PO-5).
...
All security theorems in this paper (Theorems~\ref{thm:reduction},~\ref{thm:complete-auth},~\ref{thm:net-safety}) are conditional on the deployed sighash satisfying the commitment property (Definition~\ref{def:sighash-commit}).
The Coq development now proves the modeled Sighash v2 commitment property under a SHA-256 collision-resistance axiom and exposes an extractable transcript constructor for the final preimage assembly (PO-4 in Section~\ref{sec:proof-obligations}).
The extraction pipeline compares that transcript constructor against the deployed Rust byte construction, including outpoint serialization, output serialization, spent-output serialization, and final preimage assembly with supplied sub-hashes.
What remains outside that proof is SHA-256 implementation/refinement, the SHA-256 collision-resistance assumption itself, and compiler/toolchain correctness.
Our results should be read as: ``\emph{if} the deployed sighash transcript corresponds to the verified model and SHA-256 satisfies the stated collision-resistance axiom, \emph{then} the stated security guarantees hold.''
A full mechanization of BIP\,341 itself would require formalizing the BIP\,341 specification and proving refinement to the deployed serialization --- a valuable but separate effort.
\end{remark}
...
\paragraph{Model 1: single-input transactions (\texttt{BitcoinPQ.tla}).}
The base model has explicit state, two output types (legacy and PQ-locked), single-input single-output transactions, migration announcement and cutover heights, and the consensus rule that rejects legacy output creation after announcement.
The specification was model-checked with TLC under the following instantiation: 2 genesis outputs (1 legacy, 1 PQ-locked), outpoint universe of size 6, migration announcement at height 1, cutover at height 2.
...
\paragraph{Model 2: multi-input transactions with value conservation (\texttt{BitcoinPQMulti.tla}).}
The extended model strengthens PO-6 by supporting multi-input (1--2 inputs) and multi-output (1--2 outputs) transactions with explicit value conservation ($\sum\textsf{inputs} \ge \sum\textsf{outputs}$, with the difference as fee), freeze enforcement after cutover, and a $\textsf{MigrationMonotonicity}$ invariant that tracks the PQ fraction $\rho(U_t)$ as a state variable and verifies it is non-decreasing after the announcement height.
The model was instantiated with 3 genesis outputs (total value 4), outpoint universe of size 8, migration announcement at height 1, and cutover at height 3.
TLC exhaustively explored \textbf{58{,}237 states (6{,}365 distinct)} to depth 10 with \textbf{zero violations} of all four invariants ($\textsf{NoDoubleSpend}$, $\textsf{ValueBound}$, $\textsf{StateConsistency}$, $\textsf{MigrationMonotonicity}$).
...
When $\textsf{AuthIntegrityPQ}$ (all outputs PQ-locked after cutover) is checked as an invariant in the base model, TLC produces a \textbf{concrete counterexample} in 5 states: a legacy output created before the announcement height survives to the cutover height without being migrated.
The extended model produces an analogous counterexample when $\textsf{FreezeEnforcement}$ is checked.
...
\paragraph{Module 1: spend predicate and witness encoding (\texttt{SpendPredPQ.v}).}
This module proves PO-1, PO-2, and PO-3 with a varint-based witness encoding model.
The witness parse function uses an axiomatized varint encoder/decoder (6 axioms capturing round-trip, positive length, trailing-data tolerance, prefix determinism, canonical uniqueness, and consumed-length equality) whose concrete discharge is faithful to Bitcoin's compact-size encoding through the single-byte and \texttt{0xFD}/u16 cases used by the current Coq model.
Properties proved:
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{PO-3 (Parse canonicality --- strengthened):} If two witnesses parse to the same $(\mathsf{pk},\sigma)$ pair, the witnesses are \emph{byte-identical}: $w_1 = w_2$.
  Machine-checked as \texttt{parse\_varint\_injective}, which is strictly stronger than the original PO-3 (which only showed the relevant regions match).
  The proof proceeds via \texttt{parse\_varint\_witness\_determines\_serialize}: any accepting witness $w$ satisfies $w = \textsf{Serialize}(\mathsf{pk},\sigma)$, establishing full structural canonicality.
  \item \textbf{Anti-malleability:} If $\SpendPredPQ(P,m,w_1)=1$ and $\SpendPredPQ(P,m,w_2)=1$ and both witnesses parse to the same $(\mathsf{pk},\sigma)$, then $w_1=w_2$.
  Machine-checked as \texttt{spend\_pred\_pq\_anti\_malleability}.
  \item \textbf{Round-trip:} $\textsf{Parse}(\textsf{Serialize}(\mathsf{pk},\sigma)) = \textsf{Some}(\mathsf{pk},\sigma)$ for all non-empty $\mathsf{pk},\sigma$.
  Machine-checked as \texttt{parse\_serialize\_round\_trip}.
\end{itemize}
...
\paragraph{Module 2: txid, UTXO transitions, and cost model (\texttt{TxidPreimage.v}, \texttt{UTXOTransitions.v}).}
This module mechanizes PO-5, PO-6, and PO-7.
The txid preimage module mechanizes the domain-separated transaction-id transcript used by the Rust \texttt{compute\_txid} path and proves structural injectivity of \texttt{txid\_preimage} over the committed \texttt{TxidShape}: version, input outpoints, outputs, and locktime.
The theorem is deliberately scoped to this projection rather than full transactions because txid computation excludes witness bytes.
The UTXO set is modeled as an association list with lookup, remove, and add operations.
The transition function $\DeltaTx$ is defined as removal of spent outpoints followed by addition of new outpoints with fresh identifiers.
For implementation correspondence, the module also exposes extractable boolean functions for duplicate-input detection, missing-input/value-conservation checks, structural migration/freeze checks, structural $\ValidTx$, sequential structural $\ValidBlk$, and block-cost checks.
Properties proved:

\begin{itemize}[leftmargin=2em]
  \item \textbf{PO-5 (Transition determinism):} $\DeltaTx$ is a pure function: identical inputs produce identical outputs.
  Machine-checked as \texttt{delta\_tx\_deterministic\_ext}.
  Additionally, \texttt{delta\_tx\_preserves\_no\_double\_spend} proves that the $\textsf{NoDoubleSpend}$ invariant is preserved across transitions.
  \item \textbf{PO-6 (Structural UTXO-domain/value/migration/freeze preservation):} valid structural block application preserves a duplicate-free UTXO domain below the next fresh-id bound, cannot increase total UTXO value, cannot increase legacy output count after $H_a$, consumes only present PQ inputs after $H_c$, and cannot increase frozen legacy count after $H_c$ when $H_a \le H_c$.
  Machine-checked as \texttt{apply\_valid\_block\_structural\_preserves\_domain\_nodup}, \texttt{apply\_valid\_block\_structural\_preserves\_total\_value}, \texttt{apply\_valid\_block\_structural\_legacy\_count\_nonincreasing\_after\_announcement}, \texttt{apply\_valid\_block\_structural\_inputs\_pq\_after\_cutover}, and \texttt{apply\_valid\_block\_structural\_frozen\_count\_nonincreasing\_after\_cutover}, with transaction-level preservation discharged by \texttt{delta\_tx\_preserves\_domain\_nodup}, \texttt{delta\_tx\_preserves\_domain\_bound}, and \texttt{delta\_tx\_preserves\_total\_value}.
  \item \textbf{PO-7 (Cost boundedness):} $\Cost(tx) \le \alpha \cdot \weight(tx)$ with $\alpha=1$.
  Machine-checked as \texttt{cost\_bounded\_by\_weight}.
  The stronger result \texttt{cost\_equals\_weight} proves exact equality: $\Cost(tx) = \weight(tx)$ for the SegWit witness discount model, with cost constants matching the implementation (input overhead 144\,WU, base overhead 40\,WU, per-output 164\,WU).
\end{itemize}

The extraction layer \texttt{TxidExtraction.v}/\texttt{txid\_refinement.ml} summarizes the txid transcript over deterministic count-delimited transaction matrices. \texttt{TransitionExtraction.v} and \texttt{transition\_refinement.ml} summarize structural transition functions and executable structural block-application final states over deterministic transaction, block, and cost matrices. The Coq development proves that \texttt{apply\_block\_transitions\_structural} is equivalent to the transition-validity boolean and that \texttt{apply\_valid\_block\_structural} returns \texttt{Some} final state exactly when \texttt{valid\_block\_structural} accepts. The matching Rust executables call deployed \texttt{txid\_preimage}, \texttt{valid\_tx\_structural}, \texttt{delta\_tx}, \texttt{apply\_block\_transitions\_structural}, \texttt{validate\_and\_apply\_block\_structural}, \texttt{valid\_block\_structural}, migration, freeze, and weight functions. The full Rust \texttt{valid\_tx}/\texttt{valid\_block} path is intentionally stronger than this PO-5 layer because it also verifies PQ witnesses. The matrix covers count-delimited txid transcripts, missing inputs, duplicate inputs, value inflation, the structurally valid PQ-spend boundary before witness verification, legacy output creation after $H_a$, frozen legacy/taproot spends at $H_c$, mixed PQ/legacy inputs, fee-preserving multi-input transactions, sequential intra-block dependencies, intra-block double spends, projected final UTXO states, and exact/over-limit block cost. The Rust layer now also exposes \texttt{TransitionKernel}, \texttt{StructuralTxReport}, \texttt{StructuralBlockReport}, and \texttt{DeployedTransitionKernel} as an explicit adapter boundary for the chosen Coq-first verified/extracted transition-core path. The extraction pipeline additionally wraps the Coq structural functions as \texttt{CoqExtractedTransitionKernel} and compares per-case \texttt{StructuralTxReport}/\texttt{StructuralBlockReport} witnesses directly against the Rust \texttt{DeployedTransitionKernel} adapter, including projected pre-states, transaction/block witnesses, projected final UTXO states, semantic diff output on mismatch, and release-binary validation with a dedicated certificate. For PO-6, the same extraction boundary emits structured UTXO-domain/value/migration/freeze invariant witnesses for theorem-covered accepted blocks, rejected-block cases, sequential intra-block dependencies, double-spend rejection, pre/post total-value observations, grace-period legacy-to-PQ migration, post-announcement legacy recreation rejection, post-cutover frozen-spend rejection, mixed-input rejection, and explicit fresh-id/collision-boundary non-applicability cases; the Rust counterpart observes the same final-state invariant fields through the structural block-application APIs and a dedicated release-binary certificate. Kani additionally checks twenty-one bounded source-level PO-5 harnesses over deployed Rust structural control flow: seven \texttt{valid\_tx\_structural} cases including the PQ-spend structural boundary, five \texttt{delta\_tx} removal/preservation/insertion/empty/full-spend-create cases, four \texttt{valid\_block\_structural} empty/sequential/rejection cases, three structural block-application final-state/projection cases, and two \texttt{TransitionKernel} adapter report/projection cases. Under \texttt{cfg(kani)}, txids use a deterministic bounded structural model and the UTXO set uses a fixed-capacity finite map. The runtime path exposes a domain-separated, self-delimiting \texttt{txid\_preimage}, computes \texttt{compute\_txid} as SHA-256 of that transcript, exposes structural final-state APIs for PO-5 refinement and full final-state APIs for operational validation with PQ witness checks, exposes a thin transition-kernel adapter contract for future extracted cores, and exposes key-sorted UTXO snapshots through an explicit \texttt{UtxoStore} extensional contract. Release-binary validators compare txid preimage, structural transition/final-state, TransitionKernel per-case reports, PO-6 invariant witnesses, and runtime store operations against Coq-extracted or independent deterministic references. The comparison is over an explicit projection from Coq natural-number outpoint identifiers to Rust \texttt{OutPoint}s; it is not a proof of SHA-256 primitive correctness or collision resistance, store backend internals, cryptographic witness verification, or compiler correctness. The selected unbounded-refinement path is Coq-first verified/extracted transition core plus thin Rust adapters; Verus, Creusot, or Prusti are reserved for adapter/store obligations rather than as the primary proof path for consensus transition semantics.

\paragraph{Module 3: varint axiom discharge with Bitcoin compact-size encoding (\texttt{VarintConcrete.v}).}
This module provides a concrete implementation of Bitcoin's compact-size varint encoding --- the actual multi-byte format used in the Rust implementation --- and proves that all 6 varint axioms from \texttt{SpendPredPQ.v} are satisfied.
The encoding covers two cases: $n \le 252$ maps to $[n]$ (1 byte); $253 \le n \le 65535$ maps to $[253;\; n \bmod 256;\; n / 256]$ (3 bytes, little-endian u16).
The decoding function enforces canonicality: it rejects non-minimal encodings (e.g., using the 3-byte prefix for values that fit in 1 byte).
The key proof technique for canonicality (Axiom~5) uses $\textsf{Nat.Div0.mod\_add}$ and $\textsf{Nat.div\_add}$ to show that $(lo + hi \cdot 256) \bmod 256 = lo$ and $(lo + hi \cdot 256) / 256 = hi$ when $lo < 256$, establishing that the decoded prefix equals the canonical encoding.
The proofs are packaged as a \texttt{VarintAxioms} record.
The module also proves concrete witness canonicality and parse injectivity for the bounded parser/serializer: every accepted concrete witness is exactly the canonical serialization of its parsed public key and signature, and two accepted witnesses with the same parsed components are byte-identical.
It further proves that the protocol witness cap used by the Rust implementation, \texttt{MAX\_WITNESS\_SIZE}=16{,}000, remains within the modeled u16 varint domain, that capped parsed or serialized concrete witnesses have public-key and signature component lengths inside that domain, and that the consensus-domain parser equals the byte-level parser below the cap while rejecting witnesses above the cap.
Additionally, the extraction pipeline produces \textbf{seven golden witness vectors} whose witness bytes are generated by a Coq-extracted serializer and verified byte-for-byte against the Rust implementation (including the ML-DSA-44 public key length 1{,}312 and signature length 2{,}420), exhaustively compares the Coq-extracted varint encoder/decoder against the deployed Rust functions over every modeled value $0,\ldots,65535$ plus canonical rejection cases, and compares the Coq-extracted single-signature witness serializer/parser/consensus-domain parser/canonicality behavior and parser decision traces against the Rust functions over a deterministic boundary and malformed-input refinement matrix plus 111{,}111 symbolic bounded witnesses over modeled-domain critical bytes.
On the Rust side, the parser is factored through an allocation-free witness layout function used by the public parser, consensus parser, trace hook, and canonicality predicates; five Kani harnesses verify bounded symbolic source-level alignment between those deployed Rust functions, including rejection before parsing for oversized consensus witnesses.
The CI pipeline additionally builds the PO-8 refinement examples as optimized release binaries, executes those compiled artifacts, compares their outputs against the Coq-extracted golden, varint-refinement, and witness-refinement summaries, and emits a certificate containing Rust toolchain metadata plus source, lockfile, binary, and generated-output hashes. Separate release-binary validators apply the same certificate pattern to the PQ profile activation summary, PO-4 sighash transcript refinement, PO-5 txid/UTXO transition/final-state plus TransitionKernel per-case report refinement, and PO-6 UTXO-domain/value/migration/freeze invariant witness refinement.
This addresses PO-8 at the bounded encoding level: the Coq model uses the same single-byte and \texttt{0xFD}/u16 compact-size formats as the Rust code for the consensus-valid witness subset, while the Rust-only \texttt{0xFE} and \texttt{0xFF} ranges remain outside the current Coq proof boundary for general-purpose CompactSize and are rejected by the executable consensus-domain witness parser.

\paragraph{Cryptographic suite profile and activation boundary.}
The implementation now separates standards alignment from consensus activation.
The Rust module \texttt{pq\_profile.rs} tracks ML-DSA-44/FIPS\,204 as the only consensus-enabled signature scheme and SLH-DSA-128s/FIPS\,205 as a conservative hash-based fallback profile that is dimensioned but not accepted by consensus without a deployed verifier and refinement artifacts.
The spend predicate checks this activation guard before signature verification; multisig additionally requires every committed public key to belong to the active consensus profile, preventing heterogeneous or partially unimplemented key sets from being interpreted as ML-DSA policies.
The Coq module \texttt{PQProfile.v} proves the corresponding control-plane facts: the ML-DSA-44 and SLH-DSA-128s witness sizes are 3{,}738 and 7{,}892 bytes under the actual serializer, both fit the 16{,}000-byte witness cap, the primary and fallback profiles have distinct standards and assumption families, and only the implemented ML-DSA-44 profile is consensus-enabled. The extraction layer additionally emits a structured PQ profile summary from the checked Coq definitions and compares it against Rust's \texttt{pq\_profile.rs}; the release-binary validator records a dedicated certificate for this activation-boundary correspondence.
This is not a proof of FIPS\,204/FIPS\,205 cryptographic security or signature-library correctness; it is a proof that agility metadata cannot silently widen consensus acceptance.
...
Detailed definitions of the status labels and residual implementation boundaries
are maintained in the repository proof-obligation ledger; the table below is a
paper-level index, not a duplicate of the full audit trail.

\begin{table}[t]
\centering
\small
\begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}X@{}}
\toprule
ID & Property & Status & Artifact \\
\midrule
PO-1 & Spend predicate totality & \textbf{Verified} & Coq (\texttt{SpendPredPQ.v}) \\
PO-2 & Spend predicate determinism & \textbf{Verified} & Coq (\texttt{SpendPredPQ.v}) \\
PO-3 & Witness parsing canonicality & \textbf{Verified (strengthened)} & Coq (\texttt{SpendPredPQ.v}) \\
PO-4 & Sighash commitment & \textbf{Model + transcript evidence} & Coq extraction + Rust PBT \\
PO-5 & Transition determinism & \textbf{Model + transition evidence} & Coq extraction + Kani + runtime validation \\
PO-6 & Invariant preservation & \textbf{TLC + Coq invariant evidence} & TLA+ + extracted/Rust witnesses \\
PO-7 & Cost boundedness & \textbf{Verified} & Coq (\texttt{UTXOTransitions.v}) \\
PO-8 & Implementation correspondence & \textbf{Bounded source/binary evidence} & Coq extraction + Kani + Rust PBT \\
Profile & Suite activation & \textbf{Verified profile boundary + executable correspondence} & Coq (\texttt{PQProfile.v}) + Coq-extracted/Rust profile summary + release certificate \\
\bottomrule
\end{tabularx}
\caption{Status index for proof obligations and cryptographic suite activation.}
\label{tab:po-status}
\end{table}
...
The Coq development covers the PQ spend predicate with varint-based witness encoding (PO-1, PO-2, PO-3), the UTXO transition function with no-double-spend preservation (PO-5), structural UTXO-domain invariant preservation, total-value non-increase, legacy-count non-increase after announcement, PQ-only accepted-input behavior after cutover, and frozen-count non-increase after cutover under explicit theorem premises (PO-6), and the cost model with exact weight equality (PO-7).
The varint axiom system is proved consistent by a concrete implementation (\texttt{VarintConcrete.v}).
PO-4 (sighash commitment) is proved for the Coq Sighash v2 model under a SHA-256 collision-resistance axiom and supported at the implementation layer by transcript refinement and executable evidence: a Coq-extracted \texttt{sighash\_preimage\_from\_hashes} function is compared against Rust's deployed outpoint serialization, output serialization, spent-output serialization, and final preimage assembly, including release-binary validation; a \texttt{verify\_sighash\_commitment\_property} function programmatically checks injectivity, cross-input separation, and field coverage for consensus-encodable transactions; and 9 property-based tests exercise these properties across hundreds of randomly generated transactions with BIP\,340-style tagged hashes for domain separation.
...
PO-5 is supported beyond the pure Coq transition theorem by a mechanized txid transcript and extracted refinement matrices: Coq \texttt{txid\_preimage}, structural \texttt{valid\_tx}, \texttt{delta\_tx}, structural \texttt{valid\_block}, \texttt{apply\_block\_transitions\_structural}, \texttt{apply\_valid\_block\_structural}, migration/freeze checks, and cost checks are summarized and compared against Rust's deployed structural entrypoints over count-delimited txid transcripts, missing-input, duplicate-input, value-inflation, frozen-spend, post-activation/post-cutover, sequential intra-block, double-spend, final-state, and block-cost boundary cases. Coq proves that the optional block-application semantics is equivalent to the boolean structural block validator. The same extracted structural functions are wrapped as a \texttt{CoqExtractedTransitionKernel} oracle and compared directly against Rust's \texttt{DeployedTransitionKernel} per-case \texttt{StructuralTxReport}/\texttt{StructuralBlockReport} witnesses, with semantic diffs by case and field on mismatch. PO-6 adds structural theorems and a witness matrix for duplicate-free UTXO-domain preservation, total-value non-increase, migration monotonicity, and freeze observables: \texttt{apply\_valid\_block\_structural\_preserves\_domain\_nodup}, \texttt{apply\_valid\_block\_structural\_preserves\_total\_value}, \texttt{apply\_valid\_block\_structural\_legacy\_count\_nonincreasing\_after\_announcement}, \texttt{apply\_valid\_block\_structural\_inputs\_pq\_after\_cutover}, and \texttt{apply\_valid\_block\_structural\_frozen\_count\_nonincreasing\_after\_cutover} prove the properties under explicit theorem premises, and \texttt{transition\_invariant\_refinement.ml}/\texttt{generate\_transition\_invariant\_refinement.rs} compare theorem-covered accepted cases, rejected blocks, fresh-id boundary cases, pre/post total-value observations, grace-period migration, and post-cutover freeze edge cases against Rust structural final-state observations. The txid, transition, transition-kernel, and transition-invariant release-binary validators emit certificates for these comparisons. Twenty-one Kani bounded source-level harnesses further check deployed structural transition behavior: seven \texttt{valid\_tx\_structural} cases, five \texttt{delta\_tx} removal/preservation/insertion/empty/full-spend-create cases, four \texttt{valid\_block\_structural} empty/sequential/rejection cases, three structural block-application final-state/projection cases, and two \texttt{TransitionKernel} adapter report/projection cases. A separate runtime validator emits a certificate for \texttt{txid\_preimage}, \texttt{compute\_txid}, canonical UTXO snapshots, and runtime \texttt{UtxoSet} insert/get/remove/\texttt{delta\_tx} behavior through the \texttt{UtxoStore} extensional contract. The remaining PO-5/PO-6 implementation boundary is SHA-256 primitive correctness/collision resistance, store backend internals, cryptographic witness verification, compiler/toolchain correctness, full witness-valid consensus invariant preservation in Coq, exact monetary-supply/fee accounting if required beyond non-increase, and verified extraction of the consensus transition core.
The current evidence for PO-8 consists of (i) all 6 varint axioms proved for a concrete bounded encoding in Coq covering the single-byte and \texttt{0xFD}/u16 CompactSize cases, (ii) direct Coq proofs that the bounded concrete parser is canonical and injective on the accepting domain, (iii) direct Coq proofs that the consensus-domain parser accepts exactly the byte-level parser below the cap, rejects above the cap, and only accepts canonical modeled-domain witnesses, (iv) exhaustive Coq-extracted vs Rust varint refinement over all modeled values $0,\ldots,65535$ plus canonical rejection cases, (v) Coq and Rust guards showing that the consensus witness cap remains inside that domain, including an executable Rust consensus-domain parser used by the spend predicate, (vi) a Coq-extracted vs Rust witness-level refinement matrix covering serialization, parsing, consensus-domain parsing, canonicality behavior, and parser decision traces over deterministic length-boundary and malformed-input representatives plus 111{,}111 symbolic bounded witnesses, (vii) seven golden witness vectors generated through the Coq-extracted serializer and verified byte-for-byte against the Rust implementation (including ML-DSA-44 key/signature lengths), (viii) five Kani harnesses that verify source-level bounded parser/layout/canonicality alignment over symbolic Rust byte arrays, (ix) release-binary translation validation against the Coq-extracted summaries with toolchain/source/binary hashes recorded, (x) Rust tests that exercise the corresponding implementation properties, and (xi) 3 adversarial boundary tests that exercise the security properties from Theorems~\ref{thm:reduction} and~\ref{thm:complete-auth} (commitment second-preimage resistance, cross-input replay resistance, cross-transaction replay resistance).
Full compiler-correctness closure remains a separate artifact class. For PO-4, the transcript/preimage serialization layer is extracted and compared against Rust, while SHA-256 itself remains an explicit cryptographic primitive boundary. For PO-5, txid, structural transition/final-state behavior, and TransitionKernel per-case reports are extracted and compared against Rust over deterministic matrices, bounded source-level transition behavior for deployed \texttt{valid\_tx\_structural}, \texttt{delta\_tx}, \texttt{valid\_block\_structural}, structural block application, and the \texttt{TransitionKernel} adapter projection is checked by Kani, and txid/store runtime behavior is release-validated against deterministic references. For PO-6, duplicate-free UTXO-domain preservation, total-value non-increase, legacy-count non-increase after announcement, PQ-only accepted-input behavior after cutover, and frozen-count non-increase after cutover are proved in Coq for accepted structural block application under explicit theorem premises and validated against Rust through per-case invariant witnesses and release-binary certificates; cryptographic witness validation, verified txid freshness, and exact monetary-supply/fee accounting if required beyond non-increase remain separate obligations. The selected path beyond this point is not to make the Rust standard-library-backed transition code the primary proof object; it is to move the consensus transition core toward verified/extracted Coq source and keep Rust adapters thin. For PO-8, the consensus-valid witness subset is bounded by u16, varint-level refinement is exhaustive, witness-level extracted-function plus consensus-domain operational-trace refinement is checked by CI, the Rust source parser is bounded-verified by Kani, and the release binaries are translation-validated over the modeled PO-8 domains. Extending the Coq model to \texttt{0xFE}/\texttt{0xFF} remains necessary only for a general-purpose CompactSize verification claim or if the witness cap is raised above 65{,}535 bytes.
...
\midrule
ML-DSA-44 (FIPS\,204) & 1{,}312 & 2{,}420 & $\sim$3{,}738 & 128 \\
ML-DSA-65 (FIPS\,204) & 1{,}952 & 3{,}309 & $\sim$5{,}267 & 192 \\
SLH-DSA-128s (FIPS\,205) & 32 & 7{,}856 & $\sim$7{,}892 & 128 \\
SLH-DSA-128f (FIPS\,205) & 32 & 17{,}088 & $\sim$17{,}124 & 128 \\
\bottomrule
\end{tabular}
...
BIP-361~\cite{bip361} separately frames quantum-resistant address migration and legacy signature sunset as a consensus-layer migration problem after a concrete post-quantum output type exists.
These proposals address the construction and deployment layers (how to encode PQ witnesses and how to migrate exposed legacy authorization state) but do not provide the formal consensus model, game-based security definitions, invariant preservation proofs, or implementation-refinement boundary that we contribute.
...
The formal artifacts --- TLC model checking of UTXO transitions (two models: single-input with 492 states and multi-input with 58{,}237 states and value conservation; zero structural invariant violations; concrete migration dilemma counterexample in both models), Coq-mechanized proofs of spend predicate totality, determinism, and parse canonicality (strengthened to full witness identity via axiomatized varint model with concrete bounded compact-size discharge, consensus-size guard, golden test vectors, exhaustive varint refinement, and witness-level consensus-domain operational-trace refinement matrices), Coq-mechanized PQ profile activation facts with Coq-extracted vs Rust profile-summary refinement and release-binary certification, Kani source-level bounded verification of the deployed Rust witness parser layout/canonicality relation and PO-5 structural transition behavior over \texttt{valid\_tx\_structural}, \texttt{delta\_tx}, \texttt{valid\_block\_structural}, structural block application, and \texttt{TransitionKernel} report projection, release-binary translation validation for PO-8, Coq-mechanized proofs of the Sighash v2 commitment model under a SHA-256 collision-resistance axiom plus Coq-extracted vs Rust transcript/preimage refinement for PO-4, Coq-mechanized txid preimage injectivity over the committed txid shape plus Coq-extracted vs Rust txid/structural transition/final-state and TransitionKernel per-case report refinement for PO-5, Coq-mechanized structural UTXO-domain preservation, total-value non-increase, migration monotonicity, and freeze observables plus Coq-extracted vs Rust PO-6 invariant witness refinement, runtime txid/store refinement validation for the deployed transition path, and cost boundedness with exact weight equality (PO-7), and a reference implementation in Rust with FIPS\,204 ML-DSA-44 signatures, 34 property-based tests including 9 sighash commitment properties, 28 PO-8 correspondence tests with bounded golden vectors, a Rust consensus-size guard, and 3 adversarial boundary tests --- provide machine-checked and empirically validated evidence while leaving cryptographic primitive implementation correctness, verified extraction of the transition core, SHA-256 collision-resistance implementation correctness, full witness-valid consensus invariant preservation, exact monetary-supply/fee accounting if required beyond non-increase, store backend internals, and compiler correctness as explicit implementation-correspondence boundaries.
