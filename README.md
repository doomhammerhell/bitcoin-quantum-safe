...
3. **Coq mechanized artifacts** for the spend predicate, UTXO transition, cost model, sighash model, txid preimage transcript, executable structural block-application semantics, Coq-extracted/Rust sighash transcript refinement, Coq-extracted/Rust txid/UTXO structural transition/final-state refinement, bounded PO-8 extraction evidence, Kani source-level bounded Rust refinement harnesses for the witness parser and PO-5 structural transition functions, runtime txid/UTXO-store refinement validation, and compiled-artifact translation validation against the Coq-extracted summaries.
...
| PO-5 | Transition determinism | **Verified model + txid/transition refinement + bounded source-level/runtime evidence** | Coq `UTXOTransitions.v` + Coq `TxidPreimage.v` + Coq-extracted/Rust txid and structural transition/final-state refinement over edge-case matrices + Kani bounded `valid_tx_structural`/`delta_tx`/`valid_block_structural`/structural block-application harnesses + release txid/store refinement validation |
...
| Source-level refinement | 24 | 5 Kani symbolic bounded harnesses over Rust witness parser/layout/canonicality + 19 bounded PO-5 transition harnesses over `valid_tx_structural`, `delta_tx`, `valid_block_structural`, and structural block application |
