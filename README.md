...
3. **Coq mechanized artifacts** for the spend predicate, UTXO transition, cost model, sighash model, txid preimage transcript, Coq-extracted/Rust sighash transcript refinement, Coq-extracted/Rust txid/UTXO transition refinement, bounded PO-8 extraction evidence, Kani source-level bounded Rust refinement harnesses for the witness parser and PO-5 transition functions, runtime txid/UTXO-store refinement validation, and compiled-artifact translation validation against the Coq-extracted summaries.
...
| PO-5 | Transition determinism | **Verified model + txid/transition refinement + bounded source-level/runtime evidence** | Coq `UTXOTransitions.v` + Coq `TxidPreimage.v` + Coq-extracted/Rust txid and transition refinement over structural edge-case matrices + Kani bounded `valid_tx`/`delta_tx`/`valid_block` harnesses + release txid/store refinement validation |
