...
3. **Coq mechanized artifacts** for the spend predicate, UTXO transition, cost model, sighash model, txid preimage transcript, executable block-application semantics, Coq-extracted/Rust sighash transcript refinement, Coq-extracted/Rust txid/UTXO transition/final-state refinement, bounded PO-8 extraction evidence, Kani source-level bounded Rust refinement harnesses for the witness parser and PO-5 transition functions, runtime txid/UTXO-store refinement validation, and compiled-artifact translation validation against the Coq-extracted summaries.
...
| Transition determinism | PO-5 | Coq + Coq-extracted/Rust txid and transition refinement |
...
| PO-5 | Transition determinism | **Verified model + txid/transition refinement + bounded source-level/runtime evidence** | Coq `UTXOTransitions.v` + Coq `TxidPreimage.v` + Coq-extracted/Rust txid and transition/final-state refinement over structural edge-case matrices + Kani bounded `valid_tx`/`delta_tx`/`valid_block`/block-application harnesses + release txid/store refinement validation |
...
│   └── tests/mod.rs                # 279 unit + property-based tests
├── tests/
...
The implementation uses the `fips204` crate for NIST FIPS 204 ML-DSA-44 signatures (not the pre-standard Dilithium2). The Kani harnesses verify the deployed Rust parser source over bounded symbolic byte arrays and check that the allocation-free layout parser, public `parse_witness`, consensus-domain parser, trace instrumentation, and canonicality predicates remain aligned. They also verify bounded source-level PO-5 transition behavior: `valid_tx` structural guards, `delta_tx` removal/preservation/insertion/empty/full-spend-create cases, `valid_block` empty/sequential/rejection cases, and block-application final-state projection. Under `cfg(kani)`, txids use a deterministic bounded structural model and UTXO sets use a fixed-capacity finite map. The runtime path exposes a domain-separated, self-delimiting `txid_preimage`, computes `compute_txid` as SHA-256 of that transcript, exposes `apply_block_transitions`/`validate_and_apply_block` as final-state APIs, and exposes deterministic UTXO snapshots through an explicit `UtxoStore` extensional contract. `verify_txid_refinement.sh` validates the optimized txid preimage binary against the Coq-extracted txid transcript summary. `verify_runtime_refinement.sh` validates the optimized release binary against independent references for txid preimage/hash wiring and runtime `UtxoSet` insert/get/remove/`delta_tx` behavior. The compiled-refinement scripts are run after `build_extraction.sh`: `verify_compiled_refinement.sh` validates optimized release binaries against the generated Coq-extracted PO-8 summaries, `verify_sighash_refinement.sh` validates the optimized Rust sighash transcript generator against the Coq-extracted PO-4 transcript summary, `verify_txid_refinement.sh` validates the optimized Rust txid preimage generator against the Coq-extracted txid summary, and `verify_transition_refinement.sh` validates the optimized Rust UTXO transition/final-state refinement generator against the Coq-extracted PO-5 transition/final-state summary. They emit certificates with toolchain, source, lockfile, binary, and generated-output hashes. This is translation validation and runtime refinement evidence, not a proof of `rustc`, LLVM, SHA-256 primitive correctness, or store backend internals.
...
| Validation tests | 24 | ValidTx, DeltaTx, ValidBlock, block application, txid preimage |
...
| Source-level refinement | 22 | 5 Kani symbolic bounded harnesses over Rust witness parser/layout/canonicality + 17 bounded PO-5 transition harnesses over `valid_tx`, `delta_tx`, `valid_block`, and block application |
| Compiled-artifact refinement | 5 | Release-binary validation against Coq-extracted PO-8, PO-4 transcript, PO-5 txid and transition summaries, plus runtime txid/store references |
